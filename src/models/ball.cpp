...
bool Ball::checkCollision(const shared_ptr<Rectangle> &rect) const {
...
double Ball::getCollDist(const shared_ptr<Rectangle> &rect) const {
  return (center - getCollPosOnRect(rect)).getModule() - radius;
}

void Ball::collide(const shared_ptr<Bounceable> &bounceable) {
